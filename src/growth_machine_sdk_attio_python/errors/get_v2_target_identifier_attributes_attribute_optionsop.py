"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from growth_machine_sdk_attio_python import utils
from growth_machine_sdk_attio_python.models import (
    get_v2_target_identifier_attributes_attribute_optionsop as models_get_v2_target_identifier_attributes_attribute_optionsop,
)
from growth_machine_sdk_attio_python.types import BaseModel


class GetV2TargetIdentifierAttributesAttributeOptionsNotFoundErrorData(BaseModel):
    status_code: float

    type: models_get_v2_target_identifier_attributes_attribute_optionsop.GetV2TargetIdentifierAttributesAttributeOptionsType

    code: models_get_v2_target_identifier_attributes_attribute_optionsop.GetV2TargetIdentifierAttributesAttributeOptionsCode

    message: str


class GetV2TargetIdentifierAttributesAttributeOptionsNotFoundError(Exception):
    r"""Not Found"""

    data: GetV2TargetIdentifierAttributesAttributeOptionsNotFoundErrorData

    def __init__(
        self, data: GetV2TargetIdentifierAttributesAttributeOptionsNotFoundErrorData
    ):
        self.data = data

    def __str__(self) -> str:
        return utils.marshal_json(
            self.data, GetV2TargetIdentifierAttributesAttributeOptionsNotFoundErrorData
        )
